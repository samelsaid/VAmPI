...
class User(db.Model):
    ...
    def encode_auth_token(self, user_id):
        try:
            payload = {
                ...
                + datetime.timedelta(days=0, seconds=alive),
                "iat": datetime.datetime.utcnow(),
                "sub": user_id,
            }
            return jwt.encode(
                payload, vuln_app.app.config.get("SECRET_KEY"), algorithm="HS256"
            )
    ...
    def decode_auth_token(auth_token):
        try:
            payload = jwt.decode(
                auth_token, vuln_app.app.config.get("SECRET_KEY"), algorithms=["HS256"]
            )
            return payload
    ...
    def json_debug(self):
        # Credentials are never returned, even from a debug endpoint.
    ...
    def get_user(username):
        # Bound parameter regardless of the `vuln` flag: untrusted input must
        # never be concatenated into SQL text.
    ...
    def register_user(username, password, email, admin=False):
        ...
        new_user.books = [
            Book(
                book_title="bookTitle" + randomint,
